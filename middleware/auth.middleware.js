  ...
  if (token) {
    jwt.verify(token, process.env.JWT_KEY, async (err, decodedToken) => {
      if (err) {
        ...
        if (person && person.verified === true && person.role === "student") {
          res.locals.user = {
            _id: person._id,
            email: person.email,
            lastName: person.lastName,
          ...
          next();
        } else if (person && person.verified === true) {
          authError(
            res,
            personErr,
            "You can't perform this action because you are not a student"
          );
        } else {
          authError(
            res,
...
module.exports = { requireAuth, requireStudent, requireLecturer };
